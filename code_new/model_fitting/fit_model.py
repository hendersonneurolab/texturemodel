...
def fit_fwrf(args):
    ...
    def save_all(fn2save):
        ...
        'cofactor_cpu': args.cofactor_cpu,
        }
...
#     else:
    ...
    # preprocessed data files, made using code in nsd_preproc/code 
    
    # load info about images on each trial
    info_fn = os.path.join(labels_folder, 'S%d_image_info.csv'%(args.subject))
    print(info_fn)
    ...
    # load fmri data
    data_filename = os.path.join(data_folder, 'S%d_betas_avg_bigmask.hdf5'%args.subject)
    print(data_filename)
    ...
    # Then a random 10% as the "holdout" set that is used to choose ridge parameters.
    # Could choose to use a different method for the holdout set.
    splits_filename = os.path.join(stim_folder, 'Image_data_partitions.npy')
    splits = np.load(splits_filename, allow_pickle=True).item()

    si = args.subject-1
    voxel_data_trn = voxel_data[splits['is_trn'][good_values,si], :]
    voxel_data_val = voxel_data[splits['is_val'][good_values,si], :]
    voxel_data_holdout = voxel_data[splits['is_holdout'][good_values,si], :]

    n_voxels = voxel_data_trn.shape[1]   
    
    image_order = image_order[good_values]
    image_inds_trn = image_order[splits['is_trn'][good_values,si]]
    image_inds_val = image_order[splits['is_val'][good_values,si]]
    image_inds_holdout = image_order[splits['is_holdout'][good_values,si]]
...
#                                                     args)
    ...
    print('\n\nTotal time for script: %.5f minutes'%(elapsed / 60))
