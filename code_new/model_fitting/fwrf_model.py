...
class encoding_model():
    ...
    def __init__(self, \
        ...
        self.cofactor_cpu = kwargs['cofactor_cpu'] if 'cofactor_cpu' in kwargs.keys() else False
        
        print ('FWRF model code: new version')
        print ('dtype = %s' % self.dtype)
        print ('device = %s' % self.device)
        print ('cofactor_cpu = %s' % self.cofactor_cpu)
    ...
    def __init_lambda_vecs__(self, kwargs):
        ...
        # lambda_vectors handles grid search over lambdas, where there can be different 
        # lambdas applied to different features. This is banded ridge regression.
        # [n_lambda_combs x n_features]
        # where each row is a candidate set of lambdas applied to each feature.
        # if you're doing normal ridge, then it will be constant across each row.
    ...
    def __fit_one_prf__(self, mm):
        ...
        print('breaking voxels into %d batches of %d each...'%(n_voxel_batches, self.voxel_batch_size))
        ...
        for pp in range(self.n_partial_versions):
            ...
            st = time.time()
            if self.cofactor_cpu:
                print('use cpu')
                _cof = self.__cofactor_fn_cpu__(_xtrn, self.lambda_vectors[pp][:,nonzero_inds_full]) 
            else:
                print('use gpu')
                _cof = self.__cofactor_fn_gpu__(_xtrn, self.lambda_vectors[pp][:,nonzero_inds_full]) 
            elapsed = time.time() - st
            print('elapsed time for inverse: %.5f s'%(elapsed))
    ...
    def __fit_voxel_batch_shuffle__(self, _cof, _xout, \
        ...
        for bb, batch_inds in enumerate(self.shuff_batch_inds):
            ...
            _vtrn = torch_utils._to_torch(trn_data_use[:,voxel_batch_inds], device=self.device)
            f_vout = torch_utils._to_torch(out_data_use[:,voxel_batch_inds], device=self.device)
    ...
    def __fit_voxel_batch_bootstrap__(self, _xtrn, _xout, \
        ...
        for ii in range(self.n_boot_iters):
            ...
            if self.cofactor_cpu:
                ...
                _cof = self.__cofactor_fn_gpu__(_xtrn[self.boot_inds_trn[:,ii],:], self.lambda_vectors[pp][:,nonzero_inds_full])
    ...
    def __cofactor_fn_gpu__(self, _x, lambda_vectors, regularization_eps=1e-12):
        ...
        # Compute X^T @ X once (this is the expensive part)
        xtx = _x.T @ _x  # [n_features, n_features]

         # Create identity matrix for ridge term
        eye = torch.eye(_x.size(1), device=device, dtype=dtype)

        # Vectorized computation for all lambdas at once
        # Add small epsilon for numerical stability
        lambda_tensors = torch.Tensor(lambda_vectors).to(device)
        lambda_matrices = xtx[None, :, :] + (lambda_tensors[:, :, None] + regularization_eps) * eye[None, :, :]

        try:
            # Use Cholesky decomposition for better numerical stability and speed
            # This is faster than direct inverse for positive definite matrices
            inv_matrices = torch.cholesky_inverse(torch.linalg.cholesky(lambda_matrices))
        except RuntimeError:
            # Fallback to SVD-based pseudoinverse for problematic cases
            warnings.warn("Cholesky decomposition failed, using SVD pseudoinverse")
            inv_matrices = torch.linalg.pinv(lambda_matrices)

        # Compute cofactor: (X^T*X + λI)^-1 * X^T
        cofactor = torch.bmm(inv_matrices, _x.T[None, :, :].expand(lambda_tensors.shape[0], -1, -1))
        # [lambdas x features x features] x [lambdas x features x images]
    ...
    def __get_preds_one_batch__(self, features, \
        ...
        print('breaking trials into %d batches of %d each...'%(n_trial_batches, self.sample_batch_size))
